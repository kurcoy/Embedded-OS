...
#define  CPU_CRITICAL_EXIT()      { CPU_IntDisMeasStop();  \
                                    CPU_INT_EN();          }
...
*                                                 to enable assembly-version function
*
*               (b) 'cpu_core.h'/'cpu_core.c', if CPU_CFG_LEAD_ZEROS_ASM_PRESENT  NOT #define'd in 'cpu.h'
*                                                 to enable C-source-version function otherwise
*
*               See also 'cpu_core.h  FUNCTION PROTOTYPES  Note #1'.
*********************************************************************************************************
*/

void        CPU_IntDis       (void);//没保存先前状态，不建议用户态代码使用
void        CPU_IntEn        (void);//没恢复先前状态，不建议用户态代码使用
